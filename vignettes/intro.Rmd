...
```{r, echo=FALSE}
description <- readLines(system.file("DESCRIPTION", package = "ggcognigen"))
version <- gsub(
  pattern = "Version: (.*)$",
  replacement = "\\1",
  x = description[grepl("^Version", description)]
)
install_cmd <- glue::glue("`install.packages('ggcognigen_{version}.tar.gz', type = 'source')`")
knitr::asis_output(install_cmd)
```
