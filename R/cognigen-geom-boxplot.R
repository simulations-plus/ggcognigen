...
#' by the \link[graphics]{boxplot} function, and may be apparent with small samples.
#' See \link[grDevices]{boxplot.stats} for for more information on how hinge
#' positions are calculated for \link[graphics]{boxplot}.
#'
...
#'
#' @eval ggplot2:::rd_aesthetics("geom", "cognigenBoxplot")
#'
#' @seealso \link[ggplot2]{geom_quantile} for continuous \code{x},
#'   \link[ggplot2]{geom_violin} for a richer display of the distribution, and
#'   \link[ggplot2]{geom_jitter} for a useful technique for small data.
#' @inheritParams ggplot2::layer
#' @inheritParams ggplot2::geom_bar
#' @param geom,stat Use to override the default connection between
#'   `geom_cognigenBoxplot` and `stat_cognigenBoxplot`.
#' @param outlier.jitter,outlier.colour,outlier.color,outlier.fill,outlier.shape,outlier.size,outlier.stroke,outlier.alpha
...
#'   same with outliers shown and outliers hidden. Outlier can also be display with
#'   a small degree of jitter by setting \code{outlier.jitter = TRUE}.
#'
...
#' p + geom_cognigenBoxplot()
#' p + geom_cognigenBoxplot(outlier.jitter = TRUE, coef = 90)
#' # Orientation follows the discrete axis
#' ggplot(mpg, aes(hwy, class)) + geom_cognigenBoxplot()
#'
#' p + geom_cognigenBoxplot(notch = TRUE)
#' p + geom_cognigenBoxplot(varwidth = TRUE)
#' p + geom_cognigenBoxplot(fill = "white", colour = "#3366FF")
#' # By default, outlier points match the colour of the box. Use
#' # outlier.colour to override
#' p + geom_cognigenBoxplot(outlier.colour = "red", outlier.shape = 1)
#' # Remove outliers when overlaying boxplot with original data points
#' p + geom_cognigenBoxplot(outlier.shape = NA) + geom_jitter(width = 0.2)
#'
#' # Boxplots are automatically dodged when any aesthetic is a factor
#' p + geom_cognigenBoxplot(aes(colour = drv))
#'
...
#' ggplot(diamonds, aes(carat, price)) +
#'   geom_cognigenBoxplot(aes(group = cut_width(carat, 0.25)))
#' # Adjust the transparency of outliers using outlier.alpha
#' ggplot(diamonds, aes(carat, price)) +
#'   geom_cognigenBoxplot(aes(group = cut_width(carat, 0.25)), outlier.alpha = 0.1)
#'
#' \donttest{
#' # It is possible to draw a boxplot with your own computations if you
#' # use stat = "identity":
#' y <- rnorm(100)
#' df <- data.frame(
#'   x = 1,
...
#' }
#' }
geom_cognigenBoxplot <- function(
  mapping = NULL,
  data = NULL,
  stat = "cognigenBoxplot",
  position = "dodge2",
  ...,
  outlier.jitter = FALSE,
  outlier.colour = NULL,
  outlier.color = NULL,
  outlier.fill = NULL,
  outlier.shape = 19,
  ...
  ggplot2::layer(
    ...
    mapping = mapping,
    stat = stat,
    geom = GeomCognigenBoxplot,
    position = position,
    show.legend = show.legend,
    inherit.aes = inherit.aes,
    params = list(
      outlier.jitter = outlier.jitter,
...
#' @usage NULL
#' @export
GeomCognigenBoxplot <- ggplot2::ggproto(
  "GeomCognigenBoxplot",
  ggplot2::Geom,
  ...
  draw_group = function(data, panel_params, coord, fatten = 2,
    ...
    # this may occur when using geom_cognigenBoxplot(stat = "identity")
    if (nrow(data) != 1) {
    ...
    whiskers <- ggplot2:::new_data_frame(c(
      list(
        x = c(data$x, data$x),
